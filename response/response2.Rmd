...
The editor says \textcolor{violet}{The paper falls squarely into the call for the special issue on `Reproducibility and responsible workflow". Some additional suggestions regarding framing are provided by the reviewers and the AE.} The only comments on framing appear to come from the associate editor: \textcolor{violet}{If this was (1) a general process for demonstrating the cleaning of longitudinal data, (2) a case study for cleaning data in the classroom, or (3) a case study for using the data in the classroom the value would be clear. In its current state, however, the manuscript still partly each of the three by describing the R package they've created and, as a result, does not  fully address any of the three.}

We would argue that the current form of the paper is clearly (2), and we have added some wording in the introduction and lesson learned sections to help make this clear. XXX DC to do
...
We interpret this to mean we should change the variable name in the cleaned data, and so have done this, and then "sex" is uniformly used throughout the paper. XXX Dewi to do
...
We have added more explanation of the two variables, and suggestions of where to use one vd the other in classroom analyses. XXX Dewi to do
...
Using `rlm` with the `nest` and `map` functions from `tidyr`, efficiently and simply fit these models, and are easy to explain in a classroom setting. Similar result would result from using robustlmm, but it avoids memory and fitting issues. We have added more words to the text to explain this. XXX DC to do
...
This is more of a comment than a request to make a change, but we agree that this is an area that could be a good classroom discussion. We have added a couple of additional sentences to highlight the richness of this topic, and alternative approaches that might be explored. XXX DC to do
...
It would be nice if the database contained a broader description of race, but it only has a single variable for race. For this reason, our cleaned data reflects this, and has a single variable for race. XXX Dewi to do
...
We think it is best to keep this emphasis, because the hard work is on cleaning and tidying the data, in a reproducible manner, which is close to the mission of the special issue. We have added some additional sentences about lessons for statistics and data science educators. XXX DC to do (might need help from Emi or Nick)
...
We have changed all to "refresh". XXX Dewi 
...
Thanks for this, exactly right! We have changed the wording accordingly. XXX DC
...
We have expanded this explanation XXX Dewi
...
We have put the capitalisation back in, and changed "WHITE" to be "NBH". XXX Dewi
